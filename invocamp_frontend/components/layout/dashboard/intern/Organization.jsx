...
const Organization = () => {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg md:rounded-2xl border border-gray-200 dark:border-gray-700">
      ...
      {user !== null ? (user.user.organization_experiences.length > 0 ? (
        <div className="p-8 pb-0">
          <ol className="relative border-l border-gray-200 dark:border-gray-700 ">
            {user.user.organization_experiences.map((organization, index) => (
              <li className="mb-8 ml-6" key={index}>
                ...
                <h3 className="mb-2 md:text-xl text-md font-semibold text-gray-900 dark:text-white">
                  ...
                  <span className="bg-yellow-100 text-yellow-800 text-sm ml-1 font-medium px-2.5 py-0.5 rounded dark:bg-yellow-900 dark:text-yellow-300">
                    {organization.position}
                  </span>
                </h3>
                <time className="block mb-2 text-sm font-normal leading-none text-gray-400 dark:text-gray-300">
                  {organization.start_date} -{" "}
                  {organization.end_date ? organization.end_date : "Present"}
                </time>
